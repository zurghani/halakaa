...
export const getEnrollmentsByFilters = async (filters: getEnrollmentsFilters): Promise<Student[] | Class[]> => {
    if (filters.classId) {
        const result = await db.select().from(studentClasses).innerJoin(students, eq(studentClasses.studentId, students.id)).where(eq(studentClasses.classId, filters.classId));

        return result.map((row) => row.students);
    }
    if (filters.studentId) {
        const result = await db.select().from(studentClasses).innerJoin(classes, eq(studentClasses.classId, classes.id)).where(eq(studentClasses.studentId, filters.studentId));
