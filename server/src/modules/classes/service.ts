...
export const getClassByFilters = async (filters: getClassFilters): Promise<Class[]> => {
    ...
    const result = await db.select().from(classes).where(conditions.length > 0 ? and(...conditions) : undefined);
    return result;
...
export const updateClass = async (id: number, updates: UpdateClass): Promise<Class | undefined> => {
    const [updatedClass] = await db.update(classes).set(updates).where(eq(classes.id, id)).returning();
    return updatedClass
};
